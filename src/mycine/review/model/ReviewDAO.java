...
public class ReviewDAO {
	...
	public int getMaxRef() {
		try {
			String sql = "select max(ref) from mycine_board";
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			if (rs.next()) {
				return rs.getInt(1);
	...
	public int getFindTotalCnt(String fkey, String fvalue) {
		try {
			conn = mycine.db.DBInfo.getConn();
			String sql = "select count(*) from mycine_board where " + fkey
					+ " like " + "'%" + fvalue + "%'";
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			rs.next();
	...
	public ArrayList<ReviewDTO> reviewList(int cp, int ls) {
		try {
			...
			String sql = "select * from (select rownum as rnum,a.* from "
					...
					+ "where rnum>=(" + cp + "-1)*" + ls + "+1 and rnum<=" + cp
					+ "*" + ls;
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
	...
	/** 검색 글 목록 관련 메서드 */
	public ArrayList<ReviewDTO> reviewFind(int cp, int ls, String fkey,
			String fvalue) {
		try {
			conn = mycine.db.DBInfo.getConn();
			String sql = "select * from (select rownum as rnum,a.* from "
					+ "(select * from mycine_board where " + fkey + " like "
					+ "'%" + fvalue + "%'" + " order by idx desc)a)b "
					+ "where rnum>=(" + cp + "-1)*" + ls + "+1 and rnum<=" + cp
					+ "*" + ls;
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			ArrayList<ReviewDTO> arrfind = new ArrayList<ReviewDTO>();
			while (rs.next()) {
				int idx = rs.getInt("idx");
				String writer = rs.getString("writer");
				String subject = rs.getString("subject");
