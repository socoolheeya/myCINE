	...
	public ArrayList<ReviewCommentDTO> cmtList(String fkey){
		try {
			...
			while(rs.next()){
				...
				java.sql.Date writedate_cmt=rs.getDate("writedate_cmt");
				int recommend_cmt=rs.getInt("recommend_cmt");
				int ref=rs.getInt("ref");
				int lev=rs.getInt("lev");
				int sunbun=rs.getInt("sunbun");
				
				ReviewCommentDTO cmtdtos=new ReviewCommentDTO(idxcmt, idx, writer_cmt, 
						content_cmt, writedate_cmt, recommend_cmt, ref, lev, sunbun);
				cmtarr.add(cmtdtos);
			}
			return cmtarr;
		} catch (Exception e) {
